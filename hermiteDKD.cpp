...
void hermiteDKD( const int N, double *M, double *x, double *y, double *z, double *vx, double *vy, double *vz, double *ax, double *ay, double *az, double *jx, double *jy, double *jz, double gs, const int GN );
...
//initial acceleration calculate
   ...
   float G = 6.67e-11;
   float r[3];
   float sp = 0.01;//softening parameter (not sure)
   for (int i=0; i<N; i++){
      for (int j=0; j<N; j++){
	 for (int k=0; k<3; k++){
...
while ( t<timescale ){
//predict first drift (to be corrected)
   for (int i=0; i<N; i++){
      for (int k=0; k<3; k++){
	 position[i][k] += ( 0.5*tmin*velocity[i][k] + pow(0.5*tmin, 2)*acceleration[i][k]/2 + pow(0.5*tmin, 3)*da[i][k]/6 );
...
//Then predict the kick and the second drift
   for (int i=0; i<N; i++){
      for (int k=0; k<3; k++){
//         position[i][k] += ( 0.5*tmin*velocity[i][k] + pow(0.5*tmin, 2)*acceleration[i][k]/2 + pow(0.5*tmin, 3)*da[i][k]/6 );
         velocity[i][k] += ( tmin*ah[i][k] +  pow(tmin, 2)*dah[i][k]/2 );
         position[i][k] += ( 0.5*tmin*velocity[i][k] + pow(0.5*tmin, 2)*ah[i][k]/2 + pow(0.5*tmin, 3)*dah[i][k]/6 );
                             }
                          }
